package com.github.liaoheng.common.assists;
...
public class WakeLock {
    ...
    public boolean isScreenOn() {
        if (Build.VERSION.SDK_INT < Build.VERSION_CODES.ECLAIR_MR1) {
            L.Log.w("Log : ", "can not call isScreenOn if SDK_INT < 7 ");
            return false;
        } else {
    ...
    public void turnScreenOn() {
        //点亮亮屏
        L.Log.d("Log : ", "PowerManager.WakeLock : wakeLock.isHeld: " + wakeLock.isHeld());
        if (!wakeLock.isHeld()) {
            L.Log.d("Log : ", "PowerManager.WakeLock : 点亮屏幕");
            wakeLock.acquire();
        }
    }
    ...
    public void turnScreenOff() {
        ...
        L.Log.d("Log : ", "PowerManager.WakeLock : wakeLock.isHeld: " + wakeLock.isHeld());
        if (wakeLock.isHeld()) {
            L.Log.d("Log : ", "PowerManager.WakeLock : 灭掉屏幕");
            try {
                wakeLock.release();
            } catch (Exception e) {
                e.printStackTrace();
